...
module.exports = function(rtspServer) {
  ...
  var options = function(req, res) {
    ...
    if (req.getHeader('Apple-Challenge')) {
      ...
      var ipAddrRepr = ipaddr.parse(req.socket.address().address);
  ...
  var announceParse = function(req, res) {
    ...
    if (err) {
      ...
      rtspServer.playbackClient = req.socket;
      const args = {
        audioCodec: rtspServer.audioCodec,
        audioOptions: rtspServer.audioOptions,
        audioAesKey: rtspServer.audioAesKey,
        audioAesIv: rtspServer.audioAesIv,
      };
      rtspServer.external.emit('playbackStart',args);
      res.send();
    }
  };
