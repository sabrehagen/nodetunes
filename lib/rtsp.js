...
RtspServer.prototype.connectHandler = function(socket) {
  ...
  parser.on('message',(req,res) => {
    ...
    req.ipv6 = false;
    const addr = ipaddr.parse(socket.remoteAddress);
    if (addr.kind() != 'ipv4' && addr.isIPv4MappedAddress()) {
      req.ip = addr.toIPv4Address().toString();
    } else {
      req.ipv6 = true;
    }
...
RtspServer.prototype.getEncryptionKeys = function(args) {
  const ret = {
    key: new Buffer(this.audioAesKey).toString('hex'),
    iv: this.audioAesIv.toString('hex'),
  };
  return ret;
};

RtspServer.prototype.setEncryptionKeys = function(args) {
  // !!TODO!! This shouldnt be a string ever, but for now, good enough
  this.audioAesKey = new Buffer(args.key,'hex').toString();
  this.audioAesIv = new Buffer(args.iv,'hex');
};
